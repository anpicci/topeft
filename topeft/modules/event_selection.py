...
def _lt_z_branch_mask(lep, tau, mass_window):
    same_sign_ltau_mask = ((lep.pdgId/abs(lep.pdgId)) == tau.charge)
    zpeak_mask = (abs((lep+tau).mass - 70.0)<mass_window)
    return ak.fill_none((same_sign_ltau_mask & zpeak_mask), False)

def lt_Z_mask(lep0, lep1, tau, pt_window):
    sfosz_mask0 = _lt_z_branch_mask(lep0, tau, 20.0)
    sfosz_mask1 = _lt_z_branch_mask(lep1, tau, 15.0)
...
def get_Zlt_pt(lep0, lep1, tau0):
    lt0_pt = (lep0+tau0).pt
    lt1_pt = (lep1+tau0).pt
    sfosz_mask0 = _lt_z_branch_mask(lep0, tau0, 20.0)
    sfosz_mask1 = _lt_z_branch_mask(lep1, tau0, 15.0)
    lt_pt = ak.where(sfosz_mask0, lt0_pt, lt1_pt)
    return ak.mask(lt_pt, (sfosz_mask0 | sfosz_mask1))
