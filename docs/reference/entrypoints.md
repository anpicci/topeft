...
| Raw-count diagnostics | `--record-raw-count`; YAML `record_raw_count`; default false | Records per-bin selected nominal-MC event counts for fitting-family histograms. Ordinary full production profiles do not forward it; T0 stat-only profiles do. |
