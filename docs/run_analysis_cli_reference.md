...
| `--debug-logging` | `debug_logging` | bool | `False` | Enables the processor's debug-level instrumentation. Combine it with a logging configuration that sets the root logger to DEBUG (for example via `logging.basicConfig(level="DEBUG")`) to surface the extra messages.【F:analysis/topeft_run2/run_analysis.py†L132-L206】【F:analysis/topeft_run2/run_analysis_helpers.py†L320-L392】【F:analysis/topeft_run2/analysis_processor.py†L47-L383】 |
