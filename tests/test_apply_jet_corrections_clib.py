...
def _write_minimal_jerc(year: str = "2018", era: str = "A", *, base_dir: Optional[Path] = None) -> Path:
    ...
    if base_dir is None:
        ...
        target_dir = base_dir / "data" / "POG" / "JME" / jme_year
...
def test_apply_jet_corrections_clib_savecorr(tmp_path):
    ...
    # Use an isolated topcoffea data directory so the test never overwrites
    # repository JERC payloads when they are present.
    original_topcoffea_path = list(topcoffea.__path__)
    temp_topcoffea_path = tmp_path / "topcoffea"
    temp_topcoffea_path.mkdir(parents=True, exist_ok=True)

    topcoffea.__path__ = [str(temp_topcoffea_path)]
    try:
        _write_minimal_jerc(base_dir=temp_topcoffea_path)
        ...
        topcoffea.__path__ = original_topcoffea_path
